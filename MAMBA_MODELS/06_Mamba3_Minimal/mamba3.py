...
def segsum(x: Tensor, device: Device = None) -> Tensor:
    ...
    # Always derive mask device from x itself so it stays on CUDA when x is on CUDA,
    # regardless of whether the `device` argument was passed as None.
    _dev = x.device
    mask = torch.tril(torch.ones(T, T, dtype=torch.bool, device=_dev), diagonal=-1)
    x = x.masked_fill(~mask, 0)
    x_segsum = torch.cumsum(x, dim=-2)
    mask = torch.tril(torch.ones(T, T, dtype=torch.bool, device=_dev), diagonal=0)
